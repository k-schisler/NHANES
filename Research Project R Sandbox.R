...
NHANES_mod %>%
  filter (!is.na(Pulse)) %>%
  ggplot(aes(x=as.factor(MaritalStatus), y=log(Pulse))) + 
  geom_violin() +
...
NHANES_mod %>%
  filter (!is.na(Pulse)) %>%
  ggplot(aes(x=as.factor(MaritalStatus), y=sqrt(Pulse))) + 
  geom_violin() +
  ggtitle("Distribution of Pulse by Marital Status") +
  ylab("Square Root of Pulse") + 
  xlab("Marital Status") 
...
NHANES_mod %>%
  filter (!is.na(BPDiaAve)) %>%
  ggplot(aes(x=as.factor(MaritalStatus), y=sqrt(BPDiaAve))) + 
  geom_violin() +
  ggtitle("Distribution of Log Average Diastolic Blood Pressure") +
  ylab("log of Average Diastolic Blood Pressure") + 
  xlab("Marital Status") 

NHANES_mod %>%
  filter (!is.na(BPDiaAve)) %>%
  ggplot(aes(x=as.factor(MaritalStatus), y=log(BPDiaAve))) + 
  geom_violin() +
  ggtitle("Distribution of Log Average Diastolic Blood Pressure") +
  ylab("log of Average Diastolic Blood Pressure") + 
  xlab("Marital Status") 
...
NHANES_mod %>%
  filter (!is.na(BPDiaAve)) %>%
  mutate(BPDARef = -1*BPDiaAve) %>%
  ggplot(aes(x=as.factor(MaritalStatus), y=BPDRef)) + 
  geom_violin() +
  ggtitle("Distribution of Average Diastolic Blood Pressure") +
  ylab("Average Diastolic Blood Pressure") + 
  xlab("Marital Status") 
...
NHANES_mod %>%
  filter (!is.na(PulsePressure)) %>%
  filter (Age < 50) %>% #filter age to remove older individuals, since widows are more likely to be older, Pulse pressure increases with age
  ggplot(aes(x=as.factor(MaritalStatus), y=log(PulsePressure))) + 
  geom_violin() +
  ggtitle("Distribution of log Pulse Pressure") +
  ylab("Pulse Pressure") + 
  xlab("Marital Status") 
...
NHANES_mod <- NHANES_mod %>%
  mutate(logTC = log(TotChol))

#Violin Plot
NHANES_mod %>%
  filter (!is.na(TotChol)) %>%
  ggplot(aes(x=as.factor(Relationship), y=logTC)) + 
  geom_violin() +
  ggtitle("Distribution of log Total Cholesterol by Relationship") +
  ylab("Total Cholesterol") + 
  xlab("Relationship") 
...
NHANES_mod %>%
  filter (!is.na(TotChol)) %>%
  ggplot(aes(x=as.factor(MaritalStatus), y=logTC)) + 
  geom_violin() +
  ggtitle("Distribution of log Total Cholesterol by Marital Status") +
  ylab("Total Cholesterol") + 
  xlab("Marital Status") 
